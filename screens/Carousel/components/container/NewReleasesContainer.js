...
class NewReleasesContainer extends Component {
  static propTypes = {
    ...
    paginationNewReleases: PropTypes.array,
    isConnected: PropTypes.bool,
    currentNewReleaseExperienceStreamGUID: PropTypes.string,
    totalNewReleasesRecords: PropTypes.number,
  ...
  componentDidMount() {
    const {
      ...
      isConnected,
      currentNewReleaseExperienceStreamGUID,
    } = this.props;

    if (previousTab != 'Section' && previousEmbedTab != 'Video') {
      if (isConnected) {
        this.getNewReleasesList(false, 1);
      }
    } else {
      if (this.refs._flatListView) {
        // Auto locate experience stream
        setTimeout(() => {
          if (currentNewReleaseExperienceStreamGUID) {
            paginationNewReleases.map((item, index) => {
              if (item.ExperienceStreamGUID == currentNewReleaseExperienceStreamGUID) {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if ((languageGUID != nextProps.languageGUID) && nextProps.languageGUID) {
      this.getNewReleasesList(nextProps.pullUpRefresh, nextProps.pageNumber);
    }

    if (isConnected) {
      if (pageNumber !== nextProps.pageNumber) {
        if ((totalNewReleasesRecords > (pageNumber) * limit) && totalNewReleasesRecords > 4) {
          this.getNewReleasesList(nextProps.pullUpRefresh, nextProps.pageNumber);
        }
      }
    }
  ...
  handleLoadMore = (pullUpRefresh) => {
    ...
    if (isConnected) {
      if (!isUpdating && totalNewReleasesRecords > 4 && totalNewReleasesRecords > pageNumber * limit) {
        this.props.updatePageNumber(pullUpRefresh);
      }
    }
  };

  getNewReleasesList = (pullUpRefresh, pageNumber) => {
    const {
      download,
      userGUID,
      languageGUID,
    ...
    this.props.getNewReleaseRequest(newReleasesParameters, download.storageDownloads, pullUpRefresh);
  };
  ...
  renderExperienceStream = (newReleasesItem, userGUID) => {
    const {
  ...
  render() {
    ...
    // console.log('new releases container');

    return (
      <View style={contentStyle}>
        {
          paginationNewReleases.length > 0
            ? <FlatList
              ref='_flatListView'
...
const stateToProps = state => ({
  ...
  currentNewReleaseExperienceStreamGUID: state.carousel.currentNewReleaseExperienceStreamGUID,
  totalNewReleasesRecords: state.carousel.totalNewReleasesRecords,
  paginationNewReleases: state.carousel.paginationNewReleases,
...
const dispatchToProps = dispatch => ({
  getNewReleaseRequest: (newReleasesParameters, downloads, pullUpRefresh) => dispatch(homeActions.getNewReleaseRequest(newReleasesParameters, downloads, pullUpRefresh)),
...
export default compose(connect(stateToProps, dispatchToProps), withNavigation)(NewReleasesContainer);
